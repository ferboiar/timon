/**
 * Composable para la gestión de temas y estilos de la aplicación Timón
 * ==========================================================================================
 *
 * Este composable centraliza la configuración de temas, paletas de colores y
 * funcionalidades relacionadas con los estilos de la aplicación.
 *
 * Funcionalidad principal:
 * - Definición de presets disponibles (Aura, Lara)
 * - Configuración de paletas de colores primarios y superficies
 * - Funciones para generar presets extendidos y aplicar temas
 *
 * @returns {Object} Conjunto de constantes y funciones para gestionar temas y estilos
 */
...
export function useTheme() {
    ...
    // Opciones de modo de menú
    const menuModeOptions = ref([
        { label: 'Static', value: 'static' },
        { label: 'Overlay', value: 'overlay' }
    ]);

    // Valores predeterminados para la configuración de estilo
    const defaultConfig = {
        preset: 'Aura',
        primary: 'emerald',
        surface: 'zinc',
    ...
    // Configuración de colores de superficie
    const surfaces = ref([
        {
            name: 'slate',
    ...
    /**
     * Obtiene la configuración extendida del preset basada en el color primario
     * @param {string|Object} primaryColorInput - Nombre del color primario o el objeto color completo
     * @returns {Object} Configuración extendida del preset
     */
    const getPresetExt = (primaryColorInput) => {
        let color;

        if (typeof primaryColorInput === 'string') {
            color = primaryColors.value.find((c) => c.name === primaryColorInput);
        } else if (primaryColorInput && primaryColorInput.name) {
    ...
    /**
     * Aplica el tema con un color primario o de superficie
     * @param {string} type - Tipo de color ('primary' o 'surface')
     * @param {Object} color - Objeto de color con nombre y paleta
     */
    ...
    /**
     * Aplica todos los cambios de tema con los valores proporcionados
     * @param {Object} layoutConfig - Configuración de diseño
     */
    const applyFullTheme = (layoutConfig) => {
        // Aplicar tema oscuro
        document.documentElement.classList.toggle('app-dark', layoutConfig.darkTheme);

        // Encontrar los colores seleccionados
        ...
        $t().preset(presetValue).preset(getPresetExt(primaryColor)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    };
