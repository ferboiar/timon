...
export function useAuth() {
    ...
    // Función para obtener la configuración del preset extendido
    function getPresetExt(primaryColorName) {
    ...
    const applyUserPreferences = (prefs) => {
        ...
        // Aplicar preset
        if (prefs.preset && Object.keys(presets).includes(prefs.preset)) {
        ...
        if (prefs.primary && primaryColors.some((c) => c.name === prefs.primary)) {
            layoutConfig.primary = prefs.primary;
            const primaryColor = primaryColors.find((c) => c.name === prefs.primary);
            updatePreset(getPresetExt(prefs.primary));
        }

        // Aplicar color de superficie
        if (prefs.surface && surfaces.some((s) => s.name === prefs.surface)) {
            layoutConfig.surface = prefs.surface;
            const surfaceColor = surfaces.find((s) => s.name === prefs.surface);
        ...
        const surfacePalette = surfaces.find((s) => s.name === prefs.surface)?.palette;

        $t().preset(presetValue).preset(getPresetExt(prefs.primary)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    };
